...
page = st.session_state.get('page', 'client')
...
if page == 'client':
    ...
    **How it works:**
...
elif page == 'source_control':
    st.title("🔗 Registry: Model History & Performance")
    st.info("Browse all registered models, versions, artifacts, runs, and performance metrics. Select a model in the sidebar to view details.")
    models_resp = requests.get("http://localhost:8001/models")
    ...
    if selected_model:
        ...
        st.markdown("### Versions & Artifacts")
        for v in versions:
            v_num = v.get("version", "")
            run_id = v.get("run_id", "")
        ...
        exp_id = model_detail_json.get("experiment_id", None)
        if exp_id:
            runs_resp = requests.get(f"http://localhost:8001/experiments/{exp_id}/runs")
            runs_json = runs_resp.json()
            runs = runs_json.get("runs", [])
            if runs:
                runs_df = pd.DataFrame(runs)
                st.dataframe(runs_df)
            else:
                st.caption("No runs found for this experiment.")
        else:
            st.caption("No experiment ID found for this model.")